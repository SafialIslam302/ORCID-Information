...
def create_pdf(output_file_name: str, orcid_res: Any) -> None:
    ...
    name = Paragraph(
        f"<b>Name:</b> Given Name: "
        f"{orcid_res.given_name if orcid_res.given_name else 'Not Available'}, Family Name: "
        f"{orcid_res.family_name if orcid_res.family_name else 'Not Available'}",
        body_style
    )
    story.append(name)
...
def create_txt(output_file_name: str, orcid_res: Any) -> None:
    ...
    with open(output_file_name, 'w', encoding='utf-8') as output:
        ...
        output.writelines("Family Name: " + (orcid_res.family_name if orcid_res.family_name else '') + "\n")
        output.writelines("Given Name: " + (orcid_res.given_name if orcid_res.given_name else '') + "\n")
        output.writelines(
            "Last Modified Date: " + (
...
def create_report(orcid_data: List[Any], report_type: str) -> None:
    ...
    report_rows = []
    fieldnames = [
        'ORCID', 'Given Name', 'Family Name', 'Employments', 'Education', 'Publication Number',
    ...
    for orcid_res in orcid_data:
        orcid_info = {
            ...
            'Given Name': orcid_res.given_name if orcid_res.given_name else '',
            'Family Name': orcid_res.family_name if orcid_res.family_name else '',
            'Employments': ' | '.join([
        ...
        if orcid_res.publications:
            for idx, work in enumerate(orcid_res.publications):
                work_info = {
                    'Work Title': work.title if work.title else
                    'No Information found',
                    'Work DOI URL': list(work.url.values())[0] if
                    work.url else 'No DOI URL found',
                    'Work Publication Year': work.publicationyear if
                    work.publicationyear else 'Unknown',
                    'Work Publication Type': work.publicationtype if
                    hasattr(work, 'publicationtype') else 'Unknown',
                    'Work Citation': getattr(work, 'citation_value',
                                             'No Citation Found')
                }
